<?php
    ...
    $sqlquery="SELECT * FROM tbl_mealtype where typename='$c'";
    $result=$obj->executequery($sqlquery);
    $rows=mysqli_num_rows($result);
    if($rows==1)
    {
          echo "<script>alert('Already Exist!!');window.location='mealtype.php'</script>";
    
    }
    else
    {
       $sqlquery1="INSERT INTO tbl_mealtype (typename,photo) VALUES('$c','$i')";
